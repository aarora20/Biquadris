...
Level4::Level4(int seed) : Level{4, seed} {
...
unique_ptr<Block> Level4::generateUpdatedBlock(string type) {
  if (type != " ") {
    if (type == "I") {
    ...
    return make_unique<Astblock>(false);
  } else {
...
void Level4::setNoRandomFile(string file) {
  noRandomFile = file;
  if (noRandomFile != "") {
